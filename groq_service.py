...
[Один конкретный совет для улучшения вкуса в контексте кулинарной "триады". 1-2 предложения.]
...
- Добавь питательную ценность (белки, жиры, углеводы, калории)
...
"""
    ...
    # ==================== НОВЫЕ МЕТОДЫ ДЛЯ РАЗНЫХ ТИПОВ ЗАПРОСОВ ====================
    
    async def generate_cooking_advice(self, question: str) -> str:
        """Генерация совета по готовке"""
        safe_question = self._sanitize_input(question, max_length=300)
        
        prompt = f"""Ты опытный шеф-повар с 20-летним опытом работы в ресторанах. 
Ответь на вопрос о готовке: "{safe_question}"

🎯 ТРЕБОВАНИЯ:
1. Будь практичным и конкретным
2. Дай пошаговые инструкции если нужно
3. Упомяни частые ошибки и как их избежать
4. Добавь профессиональный лайфхак
5. Объясни принципы (почему именно так)
...
💡 <b>Профессиональный лайфхак</b>
[Один конкретный совет, который редко знают домашние повара]
...
Используй эмодзи для наглядности!"""
        ...
        raw_html = await self._send_groq_request(prompt, "Generate cooking advice", 
                                               task_type="advice", temperature=0.5, max_tokens=1500)
        
        return self._clean_html_for_telegram(raw_html)
    
    async def generate_nutrition_info(self, question: str) -> str:
        """Генерация информации о питании"""
        safe_question = self._sanitize_input(question, max_length=300)
        
        prompt = f"""Ты диетолог и эксперт по спортивному питанию с научным подходом. 
Ответь на вопрос о питании: "{safe_question}"

🎯 ТРЕБОВАНИЯ:
1. Используй научно обоснованные данные
2. Приведи конкретные цифры (БЖУ, калории, витамины, минералы)
3. Дай практические рекомендации
4. Укажи противопоказания если есть
5. Объясни механизмы (почему это работает)
6. Будь точным и понятным, избегай сложных терминов

📋 ФОРМАТ ОТВЕТА (Telegram HTML):
📊 <b>Научные данные и цифры</b>
[Конкретные цифры и факты]

🍽️ <b>Практические рекомендации</b>
[Как применять на практике]

⏰ <b>Когда и как употреблять</b>
[Лучшее время, сочетания, частота]

🚫 <b>Противопоказания и ограничения</b>
[Кому не подходит, возможные побочки]

💡 <b>Вывод</b>
[Главное что нужно знать]

Используй эмодзи для наглядности, будь объективным и полезным!"""
        
        raw_html = await self._send_groq_request(prompt, "Generate nutrition info", 
                                               task_type="nutrition", temperature=0.3, max_tokens=1800)
        
        return self._clean_html_for_telegram(raw_html)
    
    async def generate_comparison(self, question: str) -> str:
        """Генерация сравнения продуктов/блюд"""
        safe_question = self._sanitize_input(question, max_length=300)
        
        prompt = f"""Ты эксперт по питанию и кулинарии. Сравни и проанализируй по запросу: "{safe_question}"

🎯 ТРЕБОВАНИЯ:
1. Будь объективным и непредвзятым
2. Используй конкретные цифры и факты
3. Рассмотри разные аспекты (питание, вкус, цена, сложность приготовления)
4. Дай рекомендации для разных целей
5. Упомяни мифы и заблуждения если есть

📋 ФОРМАТ ОТВЕТА (Telegram HTML):
📊 <b>Сравнительная таблица</b>
[Понятная таблица с ключевыми параметрами]

🔍 <b>Ключевые отличия</b>
[Главные различия по пунктам]

🍽️ <b>Рекомендации по выбору</b>
• Для похудения: [что выбрать]
• Для набора массы: [что выбрать]
• Для экономии: [что выбрать]
• Для вкуса: [что выбрать]

⚠️ <b>Важные нюансы</b>
[На что обратить внимание, скрытые плюсы/минусы]

🎯 <b>Итог</b>
[Общий вывод и совет]

Используй эмодзи для наглядности, таблицы делай понятными!"""
        
        raw_html = await self._send_groq_request(prompt, "Generate comparison", 
                                               task_type="comparison", temperature=0.4, max_tokens=2000)
