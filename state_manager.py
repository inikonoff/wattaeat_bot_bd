...
class StateManagerDB:
    ...
    async def _ensure_cache(self, user_id: int):
        """Если кэш пуст или устарел, загружаем из БД"""
        # Проверяем, есть ли пользователь в кэше И не устарели ли данные
        now = datetime.now()
        last_access = self._cache['last_access'].get(user_id)
        cache_is_fresh = (
            user_id in self._cache['products'] and 
            last_access and 
            (now - last_access).total_seconds() < self.MAX_CACHE_AGE
        )
        
        # Обновляем время доступа
        self._cache['last_access'][user_id] = now
    ...
    async def _clean_old_cache(self):
        ...
        for user_id, last_access in list(self._cache['last_access'].items()):
            if (now - last_access).total_seconds() > self.MAX_CACHE_AGE:
                users_to_remove.append(user_id)
        
        for user_id in users_to_remove:
            for key in ['products', 'states', 'categories', 'dishes', 'current_dish', 'history', 'last_recipe_id', 'last_access']:
                if user_id in self._cache[key]:
                    del self._cache[key][user_id]
        
        if users_to_remove:
            logger.info(f"🧹 Очищено {len(users_to_remove)} устаревших сессий из кэша")
